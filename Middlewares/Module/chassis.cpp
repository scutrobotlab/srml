...
inline uint8_t CChassis::torque_optimization()
{
  ...
  if(Mode == Normal_Speed)
  {
    if(Param.torqueOptimize_flag == 1)
    {
      ...
      */
      /* X Axis */
      if(fabs(Command_Velocity.x_speed) > fabs(Last_Target.x_speed))
      { //accelerate
        if(fabs(Command_Velocity.x_speed) <= Param.launch_speed)
      ...
      if(fabs(Command_Velocity.x_speed - Last_Target.x_speed) >= dspeed)
      {
      ...
      /* Y Axis */
      if(fabs(Command_Velocity.y_speed) > fabs(Last_Target.y_speed))
      { //accelerate
        if(fabs(Command_Velocity.y_speed) <= Param.launch_speed)
          max_accceleration = Param.max_launch_acceleration;
        else
          max_accceleration = Param.max_normal_acceleration;
      }
      ...
      if(fabs(Command_Velocity.y_speed - Last_Target.y_speed) >= dspeed)
      {
...
uint8_t CChassis::speed_control()
{
  ...
  */
  for (uint8_t k = 0; k < WHEEL_NUM; ++k )
     TempBuff[k] = fabs(wheel_rpmOut[k]);
...
void CChassis::Set_AccelerationParam(int16_t launch, int16_t normal, uint16_t brake)
{
  ...
  Param.max_launch_acceleration = fabs(launch);
  Param.max_normal_acceleration = fabs(normal);
	Param.max_brake_acceleration  = fabs(brake);
}
  ...
  for(uint8_t cnt = 0; cnt < WHEEL_NUM; cnt ++)
  {
    ...
    */
    if(wheel_rpmOut[cnt] != 0)
    {
      ...
      if(fabs(delta_angle) > PI)
        steer_Set[cnt].vect_angle -= _ch_dir(delta_angle)*(2*PI - fabs(delta_angle));
      else
        steer_Set[cnt].vect_angle += delta_angle;
      
      steer_Set[cnt].vect_angleAbs = fmodf(steer_Set[cnt].vect_angle,2*PI);
      steer_Set[cnt].vect_angleAbs -= fabs(steer_Set[cnt].vect_angleAbs) > PI ? _ch_dir(steer_Set[cnt].vect_angleAbs)*2*PI : 0;
    }
    ...
    */
    ...
    { /*Steer angle is limited*/
      cal_temp = fabs(steer_Set[cnt].vect_angleAbs);
